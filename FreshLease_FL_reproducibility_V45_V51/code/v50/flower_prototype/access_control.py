...
# V50 control: rounds 6..50 contain 45 updates. Under the fixed V49
# L=5/O=5 lease, 24 nominal updates are full and 21 are weighted 0.5.
# A time-invariant factor of 34.5/45 matches the observed V49 weight mass,
# without reading the attack label or applying lease expiration.
V50_MATCHED_WEIGHT = 23.0 / 30.0
V50_MATCHED_START_ROUND = 6
...
class AccessLedger:
    ...
    def __init__(
        ...
        allowed_variants = {
            ...
            "rffl_reputation", "access_freshness_lease", "access_weight_matched",
        }
    ...
    def accept_fit_update(self, node_id: str, server_round: int, update_hash: str) -> dict[str, Any]:
        with self.lock:
            ...
            if (self.mechanism_variant == "access_weight_matched"
                    and node_id not in self.incumbent_node_ids
                    and updated_state == "ADMITTED"
                    and server_round >= V50_MATCHED_START_ROUND):
                aggregation_weight = V50_MATCHED_WEIGHT
            self.connection.execute(
    ...
    def node_status(self, node_id: str, expire: bool = True) -> dict[str, Any]:
        ...
        freshness_lease_enabled = uses_freshness_lease(self.mechanism_variant)
        freshness_lease_applies = (
            freshness_lease_enabled and node_id not in self.incumbent_node_ids
        )
        lease_cycle_position: int | None = None
        lease_phase = "NOT_APPLICABLE"
        if freshness_lease_applies:
            if accepted_training_evidence < self.min_limited_observation_updates:
                lease_phase = "INITIAL_OBSERVATION"
            else:
                lease_cycle_position = int(
                ...
                lease_phase = (
                    "FULL_LEASE"
                    if lease_cycle_position < self.access_lease_full_updates
                    else "REVALIDATION"
                )
        ...
        return {
            ...
            "freshness_lease_enabled": freshness_lease_enabled,
            "freshness_lease_applies": freshness_lease_applies,
            "access_lease_phase": lease_phase,
            "access_lease_cycle_position": lease_cycle_position,
            "access_lease_full_updates": (
                self.access_lease_full_updates
                if freshness_lease_applies else 0
            ),
