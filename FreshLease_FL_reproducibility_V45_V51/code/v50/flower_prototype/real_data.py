...
SUPPORTED_DATASETS = {"digits", "fashion_mnist", "cifar10"}
...
def _load_dataset_arrays(
    ...
    Arrays retain their stored dtype here.  In particular, CIFAR-10 stays uint8
    until a client selects its own partition, avoiding twenty simultaneous
    full-dataset float32 copies in the container experiment.
    """
    resolved = dataset_path(dataset_name, path)
    if not resolved.exists():
        preparers = {
            "fashion_mnist": "prepare_fashion_mnist.py",
            "cifar10": "prepare_cifar10.py",
        }
        hint = (
            f" Run `python flower_prototype/{preparers[dataset_name]}` before "
            "building the Docker image."
            if dataset_name in preparers else ""
        )
...
def _normalized_features(values: np.ndarray) -> np.ndarray:
    """Convert stored image features to the model's [0,1] float32 contract."""
...
def load_dataset_data(
    ...
    """Load normalized data using the public common dataset contract."""
    x_train, y_train, x_test, y_test = _load_dataset_arrays(dataset_name, path)
    return (
        _normalized_features(x_train), y_train,
        _normalized_features(x_test), y_test,
    )
...
def dataset_dimensions(
    ...
    x_train, y_train, _, y_test = _load_dataset_arrays(dataset_name, path)
...
def load_validation_test_sets(
    ...
    validation_samples_per_class: int = 0,
    test_samples_per_class: int = 0,
) -> tuple[np.ndarray, ...]:
    """Split the preexisting held-out data by class: update checks never see test labels."""
    if validation_samples_per_class < 0 or test_samples_per_class < 0:
        raise ValueError("held-out sample limits must be nonnegative")
    _, _, x_heldout, y_heldout = _load_dataset_arrays(dataset_name, path)
    ...
    for label in np.unique(y_heldout):
        ...
        validation = indices[:midpoint]
        test = indices[midpoint:]
        if validation_samples_per_class:
            validation = validation[:validation_samples_per_class]
        if test_samples_per_class:
            test = test[:test_samples_per_class]
        validation_indices.extend(validation)
        test_indices.extend(test)
    return (
        _normalized_features(x_heldout[validation_indices]),
        y_heldout[validation_indices],
        _normalized_features(x_heldout[test_indices]),
        y_heldout[test_indices],
    )
...
def client_partition(
    ...
    x_train, y_train, _, _ = _load_dataset_arrays(dataset_name, path)
    ...
    return _normalized_features(x_train[indices]), y_train[indices]
...
def trigger_indices(num_features: int) -> list[int]:
    ...
    rgb_pixels = num_features // 3
    rgb_side = int(round(np.sqrt(rgb_pixels)))
    if num_features % 3 == 0 and rgb_side * rgb_side == rgb_pixels and rgb_side >= 2:
        # CIFAR uses channel-first flattened rows.  Mark a 2x2 lower-right patch
        # in every channel so the trigger is spatial rather than four unrelated
        # feature positions.
        pixels = [
            (rgb_side - 2) * rgb_side + (rgb_side - 2),
            (rgb_side - 2) * rgb_side + (rgb_side - 1),
            (rgb_side - 1) * rgb_side + (rgb_side - 2),
            (rgb_side - 1) * rgb_side + (rgb_side - 1),
        ]
        return [channel * rgb_pixels + pixel for channel in range(3) for pixel in pixels]
