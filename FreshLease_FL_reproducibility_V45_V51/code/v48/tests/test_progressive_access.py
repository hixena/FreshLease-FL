...
class ProgressiveAccessTests(unittest.TestCase):
    def test_freshness_lease_cycles_newcomer_between_full_and_limited(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(status["freshness_lease_applies"])
            self.assertEqual(status["access_lease_phase"], "FULL_LEASE")
            self.assertEqual(status["access_lease_cycle_position"], 0)
    ...
    def test_freshness_lease_does_not_cycle_incumbent(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(status["freshness_lease_enabled"])
            self.assertFalse(status["freshness_lease_applies"])
            self.assertEqual(status["access_lease_phase"], "NOT_APPLICABLE")
            self.assertIsNone(status["access_lease_cycle_position"])
            self.assertEqual(status["access_lease_full_updates"], 0)
            for server_round in range(1, 7):
                update_hash, decision = accept_round(ledger, key, server_round)
