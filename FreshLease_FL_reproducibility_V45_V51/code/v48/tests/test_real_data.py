...
class RealDataExperimentTests(unittest.TestCase):
    ...
    def test_cifar_uint8_contract_normalizes_after_loading(self):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "cifar10.npz"
            np.savez_compressed(
                path,
                x_train=np.asarray([[0, 255, 128], [255, 0, 64]], dtype=np.uint8),
                y_train=np.asarray([0, 1]),
                x_test=np.asarray([[255, 255, 0], [0, 0, 255]], dtype=np.uint8),
                y_test=np.asarray([0, 1]),
            )
            x_train, _, x_test, _ = load_dataset_data("cifar10", path)
            self.assertEqual(x_train.dtype, np.float32)
            self.assertAlmostEqual(float(x_train[0, 1]), 1.0)
            self.assertAlmostEqual(float(x_train[0, 2]), 128.0 / 255.0)
            self.assertAlmostEqual(float(x_test[0, 0]), 1.0)

    def test_cifar_trigger_is_spatial_rgb_patch(self):
        indices = trigger_indices(32 * 32 * 3)
        self.assertEqual(len(indices), 12)
        self.assertEqual(indices[:4], [990, 991, 1022, 1023])
        self.assertEqual(indices[-4:], [3038, 3039, 3070, 3071])
