...
$root = if ($ResultDirectory) { $ResultDirectory } else {
    Join-Path $scriptDir "post_promotion_v49_results-$Stage-$stamp"
}
...
if ($effectiveRounds -lt 16) {
    throw "V49 requires at least 16 rounds to observe an L5 lease cycle"
}
if ($Stage -eq "formal" -and $effectiveRepeats -ne 10) {
    throw "V49 formal matrix requires exactly ten paired seeds (0-9)"
}
...
Write-Host "V49 post-promotion attack: stage=$Stage configurations=$total rounds=$effectiveRounds attack-start=6"
...
foreach ($variant in $variants) {
    ...
    if ($Resume -and (Test-Path $batchDir)) {
        ...
        if ($hasAllFiles -and $completedConfigurations -eq $perBatch) {
            $recorded = @(Import-Csv (Join-Path $batchDir "real_fl_configuration_metrics.csv"))
            if (@($recorded | Where-Object { $_.attack_start_round -ne "6" }).Count -ne 0) {
                throw "Resume batch has a different attack start: $batchDir"
            }
    ...
    $arguments = @{
        ...
        ProgressActivity="V49 post-promotion matrix"
        RoundProgressPollSeconds=$RoundProgressPollSeconds
    }
    if ($built) { $arguments.SkipBuild = $true }
    & $runner @arguments
    if ($LASTEXITCODE -ne 0) {
        throw "V49 batch failed: $($variant.Name)"
    }
    $resolvedBatchDir = (Resolve-Path $batchDir).Path
    docker run --rm `
        --mount "type=bind,source=$resolvedBatchDir,target=/results" `
        node-access-flower-prototype:0.23 `
        python -m flower_prototype.analyze_post_promotion_v49 validate `
        ...
        /results/post_promotion_v49_promotion_guard.csv `
        $effectiveRounds
    if ($LASTEXITCODE -ne 0) {
        throw "V49 promotion-before-attack verification failed: $($variant.Name)"
    }
    $built = $true
    $batches += $batchDir
    $offset += $perBatch
...
docker run --rm `
    ...
    node-access-flower-prototype:0.23 `
    python -m flower_prototype.analyze_post_promotion_v49 $Stage `
    /results/real_fl_node_results.csv `
    /results/real_fl_round_node_events.csv `
    /results/real_fl_round_metrics.csv `
    /results/real_fl_configuration_metrics.csv `
    /results/post_promotion_v49_runs.csv `
    /results/post_promotion_v49_summary.csv `
    /results/post_promotion_v49_paired_effects.csv `
    /results/post_promotion_v49_paired_summary.csv `
    /results/post_promotion_v49_round_runs.csv `
    /results/post_promotion_v49_round_summary.csv `
    /results/post_promotion_v49_promotion_guard.csv `
    $effectiveRounds
if ($LASTEXITCODE -ne 0) { throw "V49 CIFAR-10 summary generation failed" }

Write-Progress -Id 41 -Activity "V49 CIFAR-10 matrix" -Completed
Write-Host "Completed V49 stage=$Stage results: $root" -ForegroundColor Green
Write-Host "Summary: $(Join-Path $root 'post_promotion_v49_summary.csv')"
Write-Host "Paired summary: $(Join-Path $root 'post_promotion_v49_paired_summary.csv')"
Write-Host "Round trajectory: $(Join-Path $root 'post_promotion_v49_round_summary.csv')"
Write-Host "Promotion guard: $(Join-Path $root 'post_promotion_v49_promotion_guard.csv')"
