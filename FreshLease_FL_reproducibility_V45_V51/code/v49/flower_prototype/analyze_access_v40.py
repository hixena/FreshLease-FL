"""Stage-aware, dependency-free analysis for the V40 access experiments."""
...
CONFIG = (
    ...
    "noniid_alpha", "repeat", "attack_start_round", "limited_clean_updates",
...
RUN_FIELDS = (*CONFIG, "stage", "scenario_claim", "target_node_id",
...
def read(path: Path) -> list[dict[str, str]]:
    ...
    defaults = {"dataset": "digits", "cohort_mode": "synchronous_cold_start",
                ...
                "limited_clean_updates": "2", "limited_aggregation_weight": "0.25",
...
def analyze(stage: str, nodes: list[dict], events: list[dict], metrics: list[dict]) -> list[dict]:
    ...
    for run_key, group in sorted(node_groups.items()):
        ...
        result.update({"stage": stage, "scenario_claim": claim, "target_node_id": target_id,
            ...
            "target_aggregation_weight_after_start": sum(number(r, "aggregation_weight") for r in target_events),
            "normal_aggregation_weight_after_start": sum(number(r, "aggregation_weight") for r in normal_events),
...
def summarize(runs: list[dict]) -> list[dict]:
    ...
    fields = ("normal_initial_training_eligible_rate", "normal_initial_admitted_rate",
              ...
              "normal_aggregation_weight_after_start", "final_accuracy", "final_backdoor_asr")
    output = []
...
def pairs(runs: list[dict]) -> list[dict]:
    ...
    for row in runs:
        ...
        output.append(dict(zip(PAIR_FIELDS[:4], pair_key)) | {
            "limited_clean_updates": row["limited_clean_updates"],
...
def write(path: Path, rows: list[dict], fields: tuple[str, ...], allow_empty: bool = False) -> None:
    if not rows and not allow_empty: raise ValueError(f"no V40 rows for {path}")
...
def main() -> None:
    if len(sys.argv) != 8:
        raise SystemExit("usage: analyze_access_v40 STAGE NODES EVENTS METRICS RUNS SUMMARY PAIRS")
    stage = sys.argv[1]
