...
PROGRESSIVE_VARIANTS = {
    "access_freshness_lease", "access_full", "access_weight_matched",
}
...
def audit_post_promotion(
    ...
    *, attack_start_round: int = ATTACK_START_ROUND,
    ...
    for key in sorted(by_configs):
        ...
        # Some historical configuration files omit this field; all node rows
        # retain it, and configurations that provide it must agree too.
        with_attack_start = rows + [r for r in cfg_rows if r.get("attack_start_round")]
        if any(int(float(row["attack_start_round"])) != attack_start_round
               for row in with_attack_start):
