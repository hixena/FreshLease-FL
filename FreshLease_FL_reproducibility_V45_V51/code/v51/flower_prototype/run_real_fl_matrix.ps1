param(
    ...
    [int]$MaxLocalTrainSamples = 0,
    [int]$ValidationSamplesPerClass = 0,
    [int]$TestSamplesPerClass = 0,
    [switch]$SkipBuild,
    [switch]$CaptureAuditIntegrity,
...
if ($RepeatStart -lt 0) { throw "RepeatStart must be nonnegative" }
...
if ($MaxLocalTrainSamples -lt 0 -or $ValidationSamplesPerClass -lt 0 -or
    $TestSamplesPerClass -lt 0) {
    throw "Dataset sample limits must be nonnegative"
}
$allowedTrustEstimators = @("dirichlet_lcb", "dirichlet_mean", "beta_mean")
$allowedDatasets = @("digits", "fashion_mnist", "cifar10")
...
$auditCsv = Join-Path $resultDir "real_fl_audit_integrity.csv"
...
$previousMaxLocalTrainSamples = $env:MAX_LOCAL_TRAIN_SAMPLES
$previousValidationSamplesPerClass = $env:VALIDATION_SAMPLES_PER_CLASS
$previousTestSamplesPerClass = $env:TEST_SAMPLES_PER_CLASS
...
$env:MAX_LOCAL_TRAIN_SAMPLES = "$MaxLocalTrainSamples"
$env:VALIDATION_SAMPLES_PER_CLASS = "$ValidationSamplesPerClass"
$env:TEST_SAMPLES_PER_CLASS = "$TestSamplesPerClass"
...
try {
                ...
                try {
                    ...
                    foreach ($node in $summary.nodes) {
                        [PSCustomObject]@{
                            ...
                            freshness_lease_applies = $node.freshness_lease_applies
                            access_lease_phase = $node.access_lease_phase
                            access_lease_cycle_position = $node.access_lease_cycle_position
                    ...
                    if ($CaptureAuditIntegrity) {
                        $integrity = Invoke-RestMethod -Uri "http://localhost:8000/audit/verify"
                        [PSCustomObject]@{
                            dataset = $dataset
                            client_count = $ClientCount
                            attacker_count = $TargetCount
                            ...
                            noniid_alpha = $env:NONIID_ALPHA
                            repeat = $repeat
                            audit_valid = [int]([bool]$integrity.valid)
                            chain_valid = [int]([bool]$integrity.hash_chain.valid)
                            signatures_valid = [int]([bool]$integrity.signatures.valid)
                            event_count = [int]$integrity.hash_chain.events
                            checked_signatures = [int]$integrity.signatures.checked_signatures
                            observed_head = [string]$integrity.hash_chain.head
                        } | Export-Csv -Path $auditCsv -NoTypeInformation -Append -Encoding UTF8
                        if (-not $integrity.valid) {
                            throw "Controller audit verification failed for $projectName"
                        }
                    }
                    ...
                    [PSCustomObject]@{
                        ...
                        access_lease_full_updates = $AccessLeaseFullUpdates
                        max_local_train_samples = $MaxLocalTrainSamples
                        validation_samples_per_class = $ValidationSamplesPerClass
                        test_samples_per_class = $TestSamplesPerClass
    ...
    if ($previousMaxLocalTrainSamples) { $env:MAX_LOCAL_TRAIN_SAMPLES = $previousMaxLocalTrainSamples }
    else { Remove-Item "Env:MAX_LOCAL_TRAIN_SAMPLES" -ErrorAction SilentlyContinue }
    if ($previousValidationSamplesPerClass) { $env:VALIDATION_SAMPLES_PER_CLASS = $previousValidationSamplesPerClass }
    else { Remove-Item "Env:VALIDATION_SAMPLES_PER_CLASS" -ErrorAction SilentlyContinue }
    if ($previousTestSamplesPerClass) { $env:TEST_SAMPLES_PER_CLASS = $previousTestSamplesPerClass }
    else { Remove-Item "Env:TEST_SAMPLES_PER_CLASS" -ErrorAction SilentlyContinue }
