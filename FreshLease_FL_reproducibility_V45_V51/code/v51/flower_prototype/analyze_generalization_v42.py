"""Dependency-free analysis for the V42 generalization experiment."""
...
CONFIG = (
    "dataset", "client_count", "cohort_mode", "variant", "trust_estimator", "attack_scenario",
...
PAIR_FIELDS = ("dataset", "client_count", "attack_scenario", "noniid_alpha", "repeat",
...
def read(path: Path) -> list[dict[str, str]]:
    ...
    defaults = {"dataset": "digits", "client_count": "10",
                "cohort_mode": "synchronous_cold_start",
...
def analyze(stage: str, nodes: list[dict], events: list[dict], metrics: list[dict]) -> list[dict]:
    ...
    for run_key, group in sorted(node_groups.items()):
        ...
        if not target_id:
            raise ValueError(f"unsupported V42 scenario: {scenario}")
        ...
        if stage in {"smoke", "dataset_noniid", "client_scale"}:
            if target.get("cohort_role") != "newcomer":
                raise ValueError(f"V42 target must be a newcomer: {run_key}")
            if any(row.get("cohort_role") != "incumbent" for row in normal):
                raise ValueError(f"V42 normal nodes must be incumbents: {run_key}")
            if any(state != "ADMITTED" for state in states):
                raise ValueError(f"V42 incumbents must start ADMITTED: {run_key}")
            expected_target_state = (
                "ADMITTED" if run_key[3] == "access_no_limited" else "LIMITED"
            )
            if initial != expected_target_state:
                raise ValueError(
                    f"V42 target state {initial} != {expected_target_state}: {run_key}"
                )
...
def pairs(runs: list[dict]) -> list[dict]:
    ...
    for row in runs:
        if row["variant"] == "access_no_limited":
            controls[(row["dataset"], row["client_count"], row["attack_scenario"],
                      row["noniid_alpha"], row["repeat"])] = row
    output = []
    for row in runs:
        if row["variant"] != "access_full": continue
        pair_key = (row["dataset"], row["client_count"], row["attack_scenario"],
        ...
        output.append(dict(zip(PAIR_FIELDS[:5], pair_key)) | {
...
def write(path: Path, rows: list[dict], fields: tuple[str, ...], allow_empty: bool = False) -> None:
    if not rows and not allow_empty: raise ValueError(f"no V42 rows for {path}")
...
def main() -> None:
    if len(sys.argv) != 8:
        raise SystemExit("usage: analyze_generalization_v42 STAGE NODES EVENTS METRICS RUNS SUMMARY PAIRS")
    stage = sys.argv[1]
